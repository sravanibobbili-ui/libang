...
class iCal_Event
{
	...
	{
		// if ($this->_timeEnd) {
			$ts = date('Y-m-d H:i:s', $this->_timeStart);
			$te = date('Y-m-d H:i:s', $this->_timeEnd);
			// $date1 = new DateTime("2021-11-03");
			// $date2 = new DateTime("2021-11-10");
			// $interval = $date1->diff($date2);
			return $this->_timeEnd - $this->_timeStart;
			// $diff = abs(strtotime($this->_timeEnd) - strtotime($this->_timeStart)); 
			// return $interval;
		// }
	}
	...
	public function parse($content)
	{
		...
		if ($this->summary() === "ENGL 3354 and ENGL 4366") {
			$i = 0;
		}
		// Recurrence-Id
	...
	public function fixOccurringDate($timestamp)
	{
		if($timestamp != $this->_timeStart) {
			...
			$tsShow = date('Y-m-d H:i:s', $timestamp);
